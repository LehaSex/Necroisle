using Necroisle.DevConsole.Pooling;
...
#pragma warning restore 0414, 0067, 0649
        ...
        #region Callbacks
        /// <summary>Callback executed when the DevConsole state changes.</summary>
        public event Action OnStateChange;

        /// <summary>Callback executed when the DevConsole invokes a command.</summary>
        public event Action<string> OnInvoke;

        /// <summary>Callback executed when the DevConsole is cleared.</summary>
        public event Action OnClear;

        /// <summary>Callback executed when text has been logged to the DevConsole.</summary>
        public event Action<ILog> OnLog;

        /// <summary>Callback executed when the DevConsole is activated.</summary>
        public event Action OnActivate;

        /// <summary>Callback executed when the DevConsole is deactivated.</summary>
        ...
        private void FormatSuggestion(IDevConsoleSuggestion suggestion, bool selected, StringBuilder buffer)
        {
            if (!_theme)
            {
        ...
        [Command("DevConsole-script-extern", "Executes an external source of DevConsole script file, where each line is a separate DevConsole command.", MonoTargetType.Registry, Platform.AllPlatforms ^ Platform.WebGLPlayer)]
        ...
        private void OnEnable()
        {
            ...
            else { DisableDevConsole(); }
        }

        private bool IsSupportedState()
        {
#if DevConsole_DISABLED
            return false;
#endif
        ...
        private void DisableDevConsole()
        {
            Deactivate();
            enabled = false;
        }
