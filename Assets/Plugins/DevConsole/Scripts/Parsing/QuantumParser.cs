...
namespace Necroisle.DevConsole
{
    ...
    {
        private readonly IDevConsoleParser[] _parsers;
        private readonly IDevConsoleGrammarConstruct[] _grammarConstructs;
        private readonly ConcurrentDictionary<Type, IDevConsoleParser> _parserLookup = new ConcurrentDictionary<Type, IDevConsoleParser>();
        ...
        /// </summary>
        /// <param name="parsers">The IDevConsoleParsers to use in this Quantum Parser.</param>
        /// <param name="grammarConstructs">The IDevConsoleGrammarConstructs to use in this Quantum Parser</param>
        public QuantumParser(IEnumerable<IDevConsoleParser> parsers, IEnumerable<IDevConsoleGrammarConstruct> grammarConstructs)
        {
        ...
        /// </summary>
        public QuantumParser() : this(new InjectionLoader<IDevConsoleParser>().GetInjectedInstances(), new InjectionLoader<IDevConsoleGrammarConstruct>().GetInjectedInstances())
        {

        }

        public IDevConsoleParser GetParser(Type type)
        {
            ...
            {
                foreach (IDevConsoleParser parser in _parsers)
                {
                    try
                    {
        ...
        private IDevConsoleGrammarConstruct GetMatchingGrammar(string value, Type type)
        {
            foreach (IDevConsoleGrammarConstruct grammar in _grammarConstructs)
            {
                try
                {
        ...
        public object Parse(string value, Type type)
        {
            ...
            IDevConsoleGrammarConstruct grammar = GetMatchingGrammar(value, type);
            if (grammar != null)
            {
                try
                {
            ...
            IDevConsoleParser parser = GetParser(type);
            if (parser == null)
            {
        ...
        private static readonly string[] _defaultNamespaces = new string[] { "System", "System.Collections", "System.Collections.Generic", "UnityEngine", "UnityEngine.UI", "Necroisle.DevConsole" };
