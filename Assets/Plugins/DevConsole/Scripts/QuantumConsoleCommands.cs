using Necroisle.DevConsole.Suggestors.Tags;
...
namespace Necroisle.DevConsole
{
    public static partial class QuantumConsoleProcessor
    {
        ...
        private const string helpStr = "Welcome to Developer Console! In order to see specific help about any specific command, " +
                ...
                "Necroisle.DevConsole.QuantumRegistry.RegisterObject<T> or the runtime command 'register-object<T>'.";

        [Command("help", "Shows a basic help guide for DevConsole")]
        ...
        [CommandDescription("Generates a list of all commands currently loaded by the Developer Console")]
        [Command("commands")]
        [Command("all-commands")]
        private static string GenerateCommandList()
        {
            string output = "List of all commands loaded by the Developer Console. Use 'man' on any command to see more:";
        ...
        private static IEnumerable<string> GenerateUserCommandList()
        {
            return GetUniqueCommands()
                .Where(x => !x.MethodData.DeclaringType.Assembly.FullName.StartsWith("Necroisle.DevConsole"))
