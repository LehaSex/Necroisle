...
/*         [Header("Plant Only")]
        ...
        [MenuItem("Necroisle/Create New Object", priority = 1)]
        static void ScriptableWizardMenu()
        {
            ScriptableWizard.DisplayWizard<CreateObject>("CreateObject", "CreateObject");
        }

        void CreateFolder(string full_folder)
        {
            if (!Directory.Exists(full_folder))
                Directory.CreateDirectory(full_folder);
        }
...
/*             bool is_plant_growth = type == CreateObjectType.Plant && stages_mesh != null && stages_mesh.Length > 1;
            ...
            string obj_title = object_title;
            GameObject obj = CreateBasicObject(obj_title, mesh);
...
/*             if (type == CreateObjectType.Character)
            ...
            if (type == CreateObjectType.Character)
            {
                ...
                PlayerController control = obj.AddComponent<PlayerController>();
                control.ground_layer = LayerMask.GetMask("Floor");

                // Animator 
                Animator anim = obj.AddComponent<Animator>();
                anim.runtimeAnimatorController = settings.player_animator;
                anim.avatar = settings.player_avatar;
        ...
        private GameObject CreateBasicObject(string otitle, GameObject mesh_prefab)
        {
            ...
            msh.transform.localRotation = Quaternion.Euler(0f, 0f, 0f);
