...
namespace Necroisle.WorldGen.EditorTool
{
    ...
    public class BiomeZoneEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Find Zone Adjacents"))
            {
                myScript.FindAdjacents();
                EditorUtility.SetDirty(myScript);
                EditorSceneManager.MarkSceneDirty(EditorSceneManager.GetActiveScene());
            }

            if (GUILayout.Button("Merge Points in Adjacents"))
            {
                myScript.MergePointsInAdjacents();
                EditorUtility.SetDirty(myScript);
                EditorSceneManager.MarkSceneDirty(EditorSceneManager.GetActiveScene());
            }

            if (GUILayout.Button("Generate Biome Noise"))
            {
            ...
            {
                myScript.FixOverlappingTerrain();
                EditorUtility.SetDirty(myScript);
                EditorSceneManager.MarkSceneDirty(EditorSceneManager.GetActiveScene());
            }
