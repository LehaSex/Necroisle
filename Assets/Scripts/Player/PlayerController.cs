...
namespace Necroisle
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        private PlayerAnimationController character_anim;
        ...
        private static List<PlayerController> players_list = new List<PlayerController>();
        ...
        void Awake()
        {
            ...
            character_anim = GetComponent<PlayerAnimationController>();
            ...
            if (_qc)
            {
                _qc.OnActivate += ConsoleOpened;
                _qc.OnDeactivate += ConsoleClosed;
            }
        }
        ...
        void Update()
        {
            if (console_enabled && TheGame.Get().IsPaused() || TheGame.Get().IsPaused() || console_enabled)
                CameraController.Get().move_enabled = false;
            else 
                CameraController.Get().move_enabled = true;
        ...
        void FixedUpdate()
        {
            ...
            bool outOfBounds = transform.position.y < -0.2f;
            if (outOfBounds)
            {
                OutOfBounds();
            }
        ...
        private void CalcPlayerChunk()
        {
            Vector3 pos = transform.position;
            // Get Chuck Size from MapTerrain
            // Calculate Player's Chunk
            // 1st chunck starts at -0.5,-0,5
            PlayerChunk.x = Mathf.Floor(pos.x / chunkSize + 0.5f);
            PlayerChunk.y = Mathf.Floor(pos.z / chunkSize + 0.5f);
        }
        ...
        public static PlayerController GetNearest(Vector3 pos, float range = 999f)
        {
            PlayerController nearest = null;
            float min_dist = range;
            foreach (PlayerController unit in players_list)
            {
        ...
        {
            auto_move = false;
            Debug.Log("Player fell out of bounds, searching for ground position...");
            Vector3 groundPos = Vector3.zero;
            float searchRadius = 10f; // Радиус поиска земли вокруг персонажа
            bool foundGround = false;

            // Перебираем круговые области вокруг персонажа и ищем землю
            for (int i = 180; i < 540; i += 10) // Перебор в градусах
            {
                Vector3 direction = Quaternion.Euler(0, i, 0) * Vector3.forward; // Направление вращения
                Vector3 checkPosition = transform.position + direction * searchRadius; // Позиция для проверки

                // Проверяем, есть ли земля в данной позиции
                if (PhysTool.FindGroundPosition(checkPosition, 10f, ground_layer, out groundPos))
                {
                    foundGround = true;
                    break; // Нашли землю, завершаем цикл
                }
            }

            // Если удалось найти землю, телепортируем персонажа
            if (foundGround)
            {
            ...
            {
                // Если не удалось найти землю, возвращаемся к предыдущей позиции
        ...
        {
            FaceTorward(transform.position + CameraController.Get().GetFacingFront());
        }
        ...
        {
            bool freelook = CameraController.Get().IsFreeRotation();
        ...
        private void DetectFronted()
        {
            ...
            //Debug.DrawRay(p2, facing * radius, Color.blue);
        ...
        private Vector3 FindMovementDirection()
        {
            ...
            if (!auto_move && IsControlsEnabled())
            {
                ...
                if (mcontrols.IsJoystickActive() && !character_craft.IsBuildMode())
                {
                    ...
                    cam_move = CameraController.Get().GetRotation() * new Vector3(joystick.x, 0f, joystick.y);
                }
...
/*             float boost = 1f + character_attr.GetBonusEffectTotal(BonusType.SpeedBoost);
        ...
        public static PlayerController Get(int player_id=0)
        {
        ...
        public static List<PlayerController> GetAll()
        {
            return players_list;
        }
