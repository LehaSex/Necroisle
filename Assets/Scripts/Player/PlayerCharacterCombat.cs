...
namespace Necroisle
{
    ...
    public class PlayerCharacterCombat : MonoBehaviour
    {
        ...
        [Command("hp", "Heal the player")]
        public void Heal(int amount)
        {
            character_attr.AddAttribute(AttributeType.Health, amount);
        }

        [Command("water", "Water the player")]
        public void Water(int amount)
        {
            character_attr.AddAttribute(AttributeType.Thirst, amount);
        }

        [Command("joy", "Lifts the mood of the character")]
        public void Happiness(int amount)
        {
            character_attr.AddAttribute(AttributeType.Happiness, amount);
        }

        [Command("food", "Feed the player")]
        public void Food(int amount)
        {
            character_attr.AddAttribute(AttributeType.Hunger, amount);
        }


        [Command("onepunch", "Destroy anything in one hit")]
        public void Onepunch(bool isOnepunch)
        {
            if (isOnepunch)
            {
                attack_multiplier = 10000;
            }
            else
            {
                attack_multiplier = 5;
        ...
        private void DoAttackStrikeNoTarget()
        {
            ...
            if (equipped != null && equipped.IsRangedWeapon())
            {
                ...
                if (projectile != null)
                {
                    ...
                    bool freerotate = CameraController.Get().IsFreeRotation();
                    if (freerotate)
                        dir = CameraController.Get().GetFacingDir();
                    ...
                    if (freerotate)
                        project.SetInitialCurve(CameraController.Get().GetAimDir(character));
                }
            }
            else
            {
        ...
        public int GetAttackDamage(Destructible target)
        {
            ...
            float mult = attack_multiplier * 1f + character.Attributes.GetBonusEffectTotal(BonusType.AttackBoost, target.Selectable.groups);
