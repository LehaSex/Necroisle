...
namespace Necroisle
{
    ...
    public abstract class SAction : ScriptableObject
    {
        ...
        public virtual void DoAction(PlayerController character, Selectable select)
        {

        }

        //When using an action on a ItemData in inventory (equip/eat/etc)
        public virtual void DoAction(PlayerController character, ItemSlot slot)
        {

        }

        //Condition to check if the action is possible, override to add a condition
        public virtual bool CanDoAction(PlayerController character, Selectable select)
        {
            return true; //No condition
        }

        //Condition to check if the action is possible, override to add a condition
        public virtual bool CanDoAction(PlayerController character, ItemSlot slot)
        {
            return true; //No condition
        }
